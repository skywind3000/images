...
int main(void)
{
	...
	Mat4x4f mat_view = matrix_set_lookat({ 0, 0, -1.5f }, { 0, 0, 0 }, { 0, 1, 0 });	// 摄像机方位
	Mat4x4f mat_proj = matrix_set_perspective(3.1415926f * 0.5f, 800 / 600.0f, 0.1f, 500.0f);
	Mat4x4f mat_mvp = mat_model * mat_view * mat_proj;	// 综合变换矩阵

	const int VARYING_COLOR = 0;    // 定义一个 varying 的 key

	struct VertexAttrib { Vec4f pos; Vec4f color; };

	VertexAttrib vs_input[3];

	// 顶点数据，由 VS 读取，如有多个三角形，可每次更新 vs_input 再绘制
	VertexAttrib vertex[4] = {
	...
	{
		Vec4f pos = vs_input[index].pos * mat_mvp;  // 扩充成四维矢量并变换
